...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
(add-to-list 'auto-mode-alist '("\\.js\\'" . (lambda() (indium) )))
...
(define-key custom-keys-map (kbd "C-x C-f") 'projectile-find-file)
...
(define-key custom-keys-map (kbd "C-d") 'duplicate-line)
...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs
(require 'sidebar)
(defvar sidebar-mode-map (make-keymap) "keymap for the sidebar")
(define-key sidebar-mode-map (kbd "<right>")  'sidebar-open-line)
(define-key sidebar-mode-map (kbd "<left>")  'sidebar-up-directory)

(add-hook 'before-make-frame-hook (lambda()
(message "%s" "plop")
(setq helm-split-window-default-side 'same)

(define-key sidebar-mode-map (kbd "C-f") 'helm-swoop)
